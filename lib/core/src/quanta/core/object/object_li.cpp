...
static signed li_create_base(lua_State* L, bool single_value) {
	auto* obj = lua::new_userdata<Object>(L);
...
TOGO_LI_FUNC_DEF(reduce_time) {
	auto obj = lua::get_pointer<Object>(L, 1);
	auto t = lua::get_pointer<Time const>(L, 2);
	object::reduce_time(*obj, *t);
	return 0;
}
...
static signed TOGO_LI_FUNC(push_sub)(lua_State* L, Object* obj, Array<Object>& a, bool sv_default) {
	...
	if (lua_isnone(L, 2)) {
		...
		auto sub = lua::get_pointer<Object>(L, 2);
...
static LuaModuleFunctionArray const li_funcs{
	...
	TOGO_LI_FUNC_REF(object, __mm_ctor)
	...
	TOGO_LI_FUNC_REF(object, reduce_time)
